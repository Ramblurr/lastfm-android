...
public class Radio {
	...
	Radio(String username, String md5password) {
		...
		m_subscriber = Integer.valueOf(handshake.getValue("subscriber"))
				.intValue() == 1;
	...
	public TrackInfo[] getPlaylist() {
		...
		try {
			...
			try {
				DocumentBuilder db = DocumentBuilderFactory.newInstance()
						.newDocumentBuilder();
				Document doc = db.parse(new InputSource(xspfRequest
						.openStream()));
				Element rootElement = doc.getDocumentElement();
				Element trackListElement = (Element) rootElement
						.getElementsByTagName("trackList").item(0);
				NodeList tracks = trackListElement
		...
		TrackInfo[] trackInfoArray = new TrackInfo[trackList.size()];
		trackList.toArray(trackInfoArray);
