...
public class Album extends MusicEntry {
	...
	private static final DateFormat RELEASE_DATE_FORMAT = new SimpleDateFormat(
			"d MMM yyyy, HH:mm", Locale.ENGLISH);
	 ...
	 * @see Authenticator
	 */
	public static Result addTags(String artist, String album, String tags,
			Session session) {
		return Caller.getInstance().call("album.addTags", session, "artist",
				artist, "album", album, "tags", tags);
	}
	 ...
	 * @see Authenticator
	 */
	public static Result removeTag(String artist, String album, String tag,
			Session session) {
		return Caller.getInstance().call("album.removeTag", session, "artist",
				artist, "album", album, "tag", tag);
	}
	 ...
	 */
	public static Collection<String> getTags(String artist, String album,
			Session session) {
		Result result = Caller.getInstance().call("album.getTags", session,
	...
	public static Collection<Album> search(String album, String apiKey) {
		Result result = Caller.getInstance().call("album.search", apiKey,
				"album", album);
		DomElement matches = result.getContentElement()
				.getChild("albummatches");
	...
	static Album albumFromResult(XmlPullParser xpp) {
		Album a = new Album(null, null, null);
		try {
			...
			while (loop) {
				...
				case XmlPullParser.START_TAG:
					if (n.equals("name"))
						...
						a.artist = xpp.nextText();
					else if (n.equals("url"))
						a.url = xpp.nextText();
					else if (n.equals("listeners"))
						a.listeners = Integer.parseInt(xpp.nextText());
					else if (n.equals("releasedate"))
						a.releaseDate = RELEASE_DATE_FORMAT.parse(xpp.nextText());
					else if (n.equals("playcount"))
						a.playcount = Integer.parseInt(xpp.nextText());
					else if (n.equals("image")) {
						ImageHolder.loadImages(a, xpp);
					} else if (n.equals("streamable"))
						a.streamable = xpp.nextText().equals("1");
	...
	static Album albumFromElement(DomElement element, String artistName) {
		...
		if (element.hasChild("releasedate")) {
			try {
				album.releaseDate = RELEASE_DATE_FORMAT.parse(element
						.getChildText("releasedate"));
